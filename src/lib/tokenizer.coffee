...
class Tokenizer extends Class
  ...
  readCalc: ->
    ...
    if 'calc(' is @buffer[0..4]
      calc = new Token T.CALC, @location
      @move 5
      ...
      while token = @readToken()
        ...
        break if braces.length is 0

        tokens.push token

        if @isEndOfFile()
          @syntaxError "Unterminated calc()"
  ...
  ###
  readCall: ->
    if m = @match /^(url-prefix|url|domain|regexp|calc)\(/i
      call = new Token T.CALL, m.start
      call.name = m[1]
      @skipAllWhitespace()

      if call.name is 'calc'
        call.arguments = @readCalcArguments()
      else
